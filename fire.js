...
function advancedSearch() {
    console.log("doing an advanced search")
    // get the list of profiles, filter for each input field, store in a list, load the first n
    var cardLoader = document.getElementById("profileLoader")
    if(cardLoader == null)
        return
    ...
    firebase.database().ref("users").once("value",(snapshot)=>{
        ...
        // filter the matching users into an array   
        filteredSnapshots = filterByName(usersSnapshot, document.getElementById("nameSearchInput").value.toLowerCase())
        filteredSnapshots = filterByGender(filteredSnapshots, document.getElementById("genderSearchInput").value)
        filteredSnapshots = filterByAge(filteredSnapshots, document.getElementById("ageLowerInput").value, document.getElementById("ageUpperInput").value)
        filteredSnapshots = filterByTier(filteredSnapshots, document.getElementById("tierSearchInput").value)
        
        filteredSnapshots.forEach(snap =>{
            lastLoginDays(snap)
        })
        //lastLoginDays(filteredSnapshots[0])

        // display matching users in console
        console.log("filter results: "+filteredSnapshots)

        // display a subset of the filtered profiles in cards
...
function filterByName(array, name) {    
    ...
    array.forEach(snapshot =>{
        var snapshotName = snapshot.child("name").val().toLowerCase()
        if(snapshotName.includes(name)){
            //console.log(name+" is in "+snapshotName)
            tempSnapshots.push(snapshot)            
        }
    })
    return tempSnapshots
}
function filterByTier(array, tier) {    
    ...
    var tempSnapshots = []
    array.forEach(snapshot =>{
        var snapshotTier = snapshot.child("tier").val().toLowerCase()
        if(snapshotTier.includes(tier)){
            //console.log(name+" is in "+snapshotName)
            tempSnapshots.push(snapshot)            
        }
    })
...
function filterByGender(array, gender) {    
    ...
    var tempSnapshots = []
    array.forEach(snapshot =>{
        var snapshotGender = snapshot.child("gender").val().toLowerCase()
        if(snapshotGender.includes(gender)){
            //console.log(name+" is in "+snapshotName)
            tempSnapshots.push(snapshot)            
        }
    })
...
function filterByAge(array, ageLower, ageUpper) {
    var tempSnapshots = []    
    var al = ageLower.toLowerCase(), au = ageUpper.toLowerCase()
    //console.log("age Lower: "+al+" age Upper: "+au)
    if(al == "any" && au == "any"){
        return array
    }
    else if(al == "any"){
        ...
        array.forEach(snapshot =>{
            var age = snapshot.child("age").val()        
            if(age > al && age < ageUpper)
                tempSnapshots.push(snapshot)
        })    
    }
    return tempSnapshots
}
// returns the number of days since the user has logged in
function lastLoginDays(snapshot) {
    
    // only userss will have last login dates
    //if(snapshot.child("tier").val() != "user")
        //return 0
    console.log("last login date snapshot "+snapshot)
    var lastLoginDate = new Date(snapshot.child("lastLogin").val())
    console.log(lastLoginDate)
    //var lastLoginDate = snapshot.child("lastLogin").val()
    
}
function nextProfilesAdvanced() {
    // show the next n profiles from the filtered list of profiles

}
function lastProfilesAdvanced() {
    // show the next n profiles from the filtered list of profiles

}
function showProfilesAdvanced(startIndex, endIndex) {
    // show the profiles from the filtered array between the given indicies
